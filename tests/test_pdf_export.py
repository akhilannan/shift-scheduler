...
def data_manager():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as tempfile_obj:
    ...
    dm.save_schedule("2025-08", {
        "2025-08-01": {"day_shift": {"employee_id": emp.id, "is_manual": False}, "night_shift": None}
    })
    yield dm
    os.unlink(temp_path)
...
def test_pdf_export_bad_path(export_manager):
    ...
    result = export_manager.export_calendar(2025, 8, 'pdf', '/not_a_dir/this_file_should_fail.pdf')
