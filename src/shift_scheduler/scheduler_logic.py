...
class ShiftScheduler:
    ...
    def generate_schedule(
        ...
        logger.info(
            f"Starting schedule generation for {month_key} (partial_generation={partial_generation})"
        )

        if partial_generation:
            is_partial, current_day, existing_schedule, days_to_generate = (
                self._detect_partial_generation_scope(year, month)
            )

            if is_partial:
                logger.info(
                    f"Detected partial generation for {len(days_to_generate)} days."
                )
                return self._generate_schedule_cp_sat_partial(
                    year,
                    month,
                    existing_schedule,
                    current_day,
                ...
                logger.info(
                    f"No partial generation needed for {month_key}, falling back to full generation."
                )
                return self._generate_full_schedule_cp_sat(
                    year,
                    month,
                    allow_quota_violations,
                    emergency_mode,
                    warm_start,
                    start_time,
                )
    ...
    def _generate_schedule_cp_sat_partial(
        self,
        year: int,
        month: int,
        ...
        adjusted_quotas = self._calculate_partial_quotas(
            year, month, existing_schedule, current_day
        )

        # Create partial CP-SAT model
        model, variables = self._create_partial_cp_sat_model(
            year,
            month,
            existing_schedule,
            current_day,
        ...
        if success:
            partial_schedule = self._extract_partial_schedule_from_solution(
                solver, variables, year, month, existing_schedule
            )
            schedule = self._merge_partial_schedule(
                existing_schedule, partial_schedule, days_to_generate
            )
            ...
            message = (
                f"Failed to generate partial schedule for days: {days_to_generate}"
            )
        ...
        logger.info(
            f"Partial CP-SAT generation completed in {duration:.2f}s. Success: {success}"
        )
    ...
    def _generate_full_schedule_cp_sat(
        self,
        year: int,
        month: int,
        allow_quota_violations: bool,
        emergency_mode: bool,
        warm_start: bool,
        start_time: float,
    ) -> ScheduleResult:
        ...
        # Create CP-SAT model
        model, variables = self._create_cp_sat_model(
            year, month, allow_quota_violations, warm_start
        )
        logger.info(
            f"Created full CP-SAT model with {len(self.employees)} employees and {variables['num_days']} days"
        )
        ...
        if success:
            schedule = self._extract_schedule_from_solution(
                solver, variables, year, month
            )
        ...
        duration = time.time() - start_time
        logger.info(
            f"Full CP-SAT generation completed in {duration:.2f}s. Success: {success}"
        )
    ...
    def _initialize_for_month(self, year: int, month: int):
        ...
        for emp in self.employees.values():
            ...
            self.quotas[emp.name] = self.data_manager.get_bucket_quota_for_employee(
                emp.name, days_in_month
            )
    ...
    def _detect_partial_generation_scope(
        self, year: int, month: int
    ) -> Tuple[bool, int, Dict[str, Dict[str, Optional[int]]], List[int]]:
        """
        ...
        if is_current_month:
            ...
            for day in range(1, current_day + 1):
                ...
                day_assigned = (
                    day_shift_info is not None
                    and day_shift_info.get("employee_id") is not None
                )
                night_assigned = (
                    night_shift_info is not None
                    and night_shift_info.get("employee_id") is not None
                )
        ...
        return (
            ...
            current_day,
            existing_schedule,
            sorted(list(set(days_to_generate))),
        )

    def _calculate_partial_quotas(
        self,
        year: int,
        month: int,
    ...
    def _create_partial_cp_sat_model(
        self,
        year: int,
        month: int,
        existing_schedule: Dict[str, Dict[str, Optional[int]]],
        current_day: int,
        adjusted_quotas: Dict[str, int],
        days_to_generate: List[int],
    ) -> Tuple[Any, Dict]:
        """
        ...
        eligible = self._create_eligibility_matrix_partial(
            year, month, days_to_generate
        )

        # Handle cross-date constraints for partial generation
        self._handle_cross_date_constraints_partial(
            model, x, existing_schedule, year, month, days_to_generate
        )
        ...
        for emp_id in self.employees:
            for day in days_to_generate:
                model.Add(
                    x[emp_id][day][ShiftType.DAY] + x[emp_id][day][ShiftType.NIGHT] <= 1
                )
        ...
        for day in days_to_generate:
            for shift_type in [ShiftType.DAY, ShiftType.NIGHT]:
                ...
                date_str = date(year, month, day).strftime("%Y-%m-%d")
                shift_key = (
                    "day_shift" if shift_type == ShiftType.DAY else "night_shift"
                )
        ...
        for emp_id, emp in self.employees.items():
            ...
            model.Add(
                total_shifts_in_gen_days <= adjusted_quota + 5
            )  # Allow some leeway
    ...
    def _handle_cross_date_constraints_partial(
        self,
        model: Any,
        x: Dict[int, Dict[int, Dict[ShiftType, Any]]],
        existing_schedule: Dict[str, Dict[str, Optional[int]]],
        year: int,
        month: int,
        days_to_generate: List[int],
            ...
            for day in days_to_generate:
                ...
                if day > 1:
                    ...
                    if prev_day in days_to_generate:
                        ...
                        model.Add(
                            x[emp_id][day][ShiftType.DAY]
                            + x[emp_id][prev_day][ShiftType.NIGHT]
                            <= 1
                        )
                    else:
                        # Constraint is between a generated day and an existing day
                        prev_date_str = date(year, month, prev_day).strftime("%Y-%m-%d")
                        prev_night_emp = existing_schedule.get(prev_date_str, {}).get(
                            "night_shift"
                        )
                        if (
                            prev_night_emp
                            and prev_night_emp.get("employee_id") == emp_id
                        ):
                ...
                if day < days_in_month:
                    ...
                    if next_day in days_to_generate:
                        model.Add(
                            x[emp_id][day][ShiftType.NIGHT]
                            + x[emp_id][next_day][ShiftType.NIGHT]
                            <= 1
                        )
                    else:
                        next_date_str = date(year, month, next_day).strftime("%Y-%m-%d")
                        next_night_emp = existing_schedule.get(next_date_str, {}).get(
                            "night_shift"
                        )
                        if (
                            next_night_emp
                            and next_night_emp.get("employee_id") == emp_id
                        ):
                            model.Add(x[emp_id][day][ShiftType.NIGHT] == 0)

    def _create_eligibility_matrix_partial(
        self, year: int, month: int, days_to_generate: List[int]
    ) -> Dict[int, Dict[int, Dict[ShiftType, bool]]]:
        ...
        for emp_id, emp in self.employees.items():
            ...
            for day in days_to_generate:
                ...
                for shift_type in [ShiftType.DAY, ShiftType.NIGHT]:
                    eligible[emp_id][day][shift_type] = (
                        self._is_employee_eligible_for_shift(
                            emp_id, shift_date, shift_type
                        )
                    )

        return eligible

    def _extract_partial_schedule_from_solution(
        self,
        solver: Any,
        variables: Dict,
        year: int,
    ...
    def _merge_partial_schedule(
        ...
        for day in days_to_generate:
            date_str = date(
                int(list(partial_schedule.keys())[0][:4]),
                int(list(partial_schedule.keys())[0][5:7]),
                day,
            ).strftime("%Y-%m-%d")
            ...
            if merged[date_str].get("day_shift") is None and partial_schedule.get(
                date_str, {}
            ).get("day_shift"):
                merged[date_str]["day_shift"] = partial_schedule[date_str]["day_shift"]
            if merged[date_str].get("night_shift") is None and partial_schedule.get(
                date_str, {}
            ).get("night_shift"):
                merged[date_str]["night_shift"] = partial_schedule[date_str][
                    "night_shift"
                ]

        return merged

    def _create_cp_sat_model(
        self,
        year: int,
        month: int,
        allow_quota_violations: bool,
        ...
        for emp_id in self.employees:
            for day in range(2, days_in_month + 1):  # Start from day 2
                model.Add(
                    x[emp_id][day][ShiftType.DAY] + x[emp_id][day - 1][ShiftType.NIGHT]
                    <= 1
                )
        ...
        for emp_id in self.employees:
            for day in range(2, days_in_month + 1):
                model.Add(
                    x[emp_id][day][ShiftType.NIGHT]
                    + x[emp_id][day - 1][ShiftType.NIGHT]
                    <= 1
                )
        ...
        for emp_id, emp in self.employees.items():
            ...
            deviation = model.NewIntVar(
                -days_in_month * 2, days_in_month * 2, f"dev_{emp_id}"
            )
        ...
        # Store variables for later use
        variables = {"x": x, "num_days": days_in_month, "num_employees": num_employees}
        return model, variables

    def _set_warm_start_hints(
        self,
        model: Any,
        x: Dict[int, Dict[int, Dict[ShiftType, Any]]],
        year: int,
            ...
            for shift_type in [ShiftType.DAY, ShiftType.NIGHT]:
                ...
                if prior_shift_info is not None:
                    ...
                    if prior_emp_id is not None and prior_emp_id in self.employees:
                        ...
                        logger.debug(
                            f"Hint: {self.employees[prior_emp_id].name} assigned to {shift_type.value} on {date_str}"
                        )
        ...
        for emp_id, emp in self.employees.items():
            ...
            for day in range(1, days_in_month + 1):
                ...
                for shift_type in [ShiftType.DAY, ShiftType.NIGHT]:
                    ...
                    is_eligible = self._is_employee_eligible_for_shift(
                        emp_id, shift_date, shift_type
                    )
    ...
    def _is_employee_eligible_for_shift(
        self, emp_id: int, shift_date: date, shift_type: ShiftType
    ) -> bool:
    ...
    def _solve_cp_sat_model(
        self, model: Any, variables: Dict, time_limit_seconds: float = 30.0
    ) -> Optional[Any]:
        ...
        if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
            ...
            logger.warning(
                f"CP-SAT solver failed with status: {solver.StatusName(status)}"
            )
            return None

    def _extract_schedule_from_solution(
        self, solver: Any, variables: Dict, year: int, month: int
    ) -> Dict[str, Dict[str, Optional[int]]]:
        ...
        for day in range(1, variables["num_days"] + 1):
            shift_date = date(year, month, day)
            date_str = shift_date.strftime("%Y-%m-%d")
            schedule[date_str] = {"day_shift": None, "night_shift": None}
            for emp_id in self.employees:
                if solver.Value(x[emp_id][day][ShiftType.DAY]) == 1:
                    schedule[date_str]["day_shift"] = {
                        "employee_id": emp_id,
                        "is_manual": False,
                    }
                if solver.Value(x[emp_id][day][ShiftType.NIGHT]) == 1:
                    schedule[date_str]["night_shift"] = {
                        "employee_id": emp_id,
                        "is_manual": False,
                    }
        return schedule

    def _validate_cp_sat_solution(
        self, schedule: Dict[str, Dict[str, Optional[int]]], year: int, month: int
    ) -> List[str]:
        ...
        for date_str, day_schedule in schedule.items():
            ...
            night_emp = (
                night_shift_info.get("employee_id") if night_shift_info else None
            )
            ...
            if day_emp is not None and day_emp == night_emp:
                violations.append(
                    f"Employee {day_emp} assigned to both shifts on {date_str}"
                )
            ...
            if day_emp is not None:
                ...
                if prev_night_emp and prev_night_emp.get("employee_id") == day_emp:
                    violations.append(
                        f"Employee {day_emp} assigned day shift on {date_str} after night shift on {prev_date_str}"
                    )
            ...
            if night_emp is not None:
                ...
                if prev_night_emp and prev_night_emp.get("employee_id") == night_emp:
                    violations.append(
                        f"Employee {night_emp} assigned consecutive night shifts on {prev_date_str} and {date_str}"
                    )

        return violations

    def validate_manual_assignment(
        self, emp_id: int, date_str: str, shift_type: str, current_schedule: Dict
    ) -> List[str]:
        """
        ...
        if shift_type == "night_shift":
            ...
            if next_day_emp == emp_id:
                violations.append(
                    f"{ConstraintViolation.NEXT_DAY_CONFLICT} (next day's day shift)"
                )
    ...
    def get_schedule_statistics(
        self, schedule: Dict[str, Dict[str, Optional[int]]], month_key: str
    ) -> Dict[str, Any]:
        ...
        for emp_name, emp_stats in stats["employee_stats"].items():
            emp_stats["quota_deviation"] = (
                emp_stats["total_shifts"] - emp_stats["quota"]
            )
    ...
    def suggest_schedule_improvements(
        self, schedule: Dict[str, Dict[str, Optional[int]]]
    ) -> List[str]:
        ...
        high_exp_ratio = stats["experience_distribution"]["High"] / max(
            stats["total_shifts"], 1
        )
        if high_exp_ratio < 0.6:  # High experience should handle majority
            suggestions.append(
                "Consider assigning more shifts to high experience employees"
            )
